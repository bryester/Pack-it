...
- (void)applyParameters:(NSDictionary *)parameters onRequest:(NSMutableURLRequest *)aRequest;
{
    ...
    //NSLog(@"applyParameters:parameter:%@", parameters);
    
    NSString *httpMethod = [aRequest HTTPMethod];
    
    
    //NSString *apiVersion = [aRequest valueForHTTPHeaderField:@"API-VERSION"];
    
    //NSLog(@"API-VERSION:%@", apiVersion);
    ...
    if ([httpMethod caseInsensitiveCompare:@"POST"] != NSOrderedSame
        ...
        //NSLog(@"NXOAuth2Connection - contentType:%@", contentType);
        ...
        if (!contentType || [contentType isEqualToString:@"multipart/form-data"]) {
            ...
            //NSLog(@"NXOAuth2Connection - aRequest:%@", aRequest);
            ...
            // sends the POST/PUT request as application/json
            
            //NSLog(@"NXOAuth2Connection - contentType:%@", contentType);
            
            
            
            //NSLog(@"objectforkey content-type:%@", [parameters objectForKey:@"Content-Type"]);
            ...
            NSString *str =[parameters objectForKey:@"data"];
            
            //NSLog(@"string:%@", str);
            
            NSData *jData = [str dataUsingEncoding:NSUTF8StringEncoding];
            
            [aRequest setHTTPBody:jData];

            NSString *contentLength = [NSString stringWithFormat:@"%lu", [[aRequest HTTPBody] length]];
